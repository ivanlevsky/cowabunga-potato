...
mail_driver = SeleniumUtils.init_driver('edge', GlobalParam.get_edge_driver_path())
SeleniumUtils.open_browser_multi_tab(mail_driver, mail_lists)
SeleniumUtils.wait_for_page_full_loaded(mail_driver)
...
def hoperun_login(hoperun_driver, user_name, user_pass):
    ...
    element = SeleniumUtils.find_element_by_id(hoperun_driver, 'usernameTip')
    element.click()
    element = SeleniumUtils.find_element_by_id(hoperun_driver, 'username')
    element.send_keys(user_name)
    element = SeleniumUtils.find_element_by_id(hoperun_driver, 'userType')
    element.click()
    element = SeleniumUtils.find_element_by_id(hoperun_driver, 'userTypePwd')
    element.send_keys(user_pass)
    element = SeleniumUtils.find_element_by_id(hoperun_driver, 'wmSubBtn')
    element.click()


def hoperun_check_mail(hoperun_driver, mail_sender, mail_title):
    SeleniumUtils.wait_for_frame_and_switch_to_frame(hoperun_driver, 'treeBox')
    element = SeleniumUtils.find_element_by_id(hoperun_driver, 'tree_folder_1_span')
    element.click()
    SeleniumUtils.wait_for_page_full_loaded(hoperun_driver)
    SeleniumUtils.wait_for_frame_and_switch_to_frame(hoperun_driver, 'tabsHome')
    SeleniumUtils.wait_for_page_full_loaded(hoperun_driver)
...
def qq_login(qq_driver, user_name, user_pass):
    element = SeleniumUtils.find_element_by_id(qq_driver, 'qqLoginTab')
    element.click()
    qq_driver.switch_to.frame('login_frame')
    element = SeleniumUtils.find_element_by_id(qq_driver, 'u')
    element.click()
    element.send_keys(user_name)
    element = SeleniumUtils.find_element_by_id(qq_driver, 'p')
    element.click()
    element.send_keys(user_pass)
    element = SeleniumUtils.find_element_by_id(qq_driver, 'login_button')
    element.click()
    SeleniumUtils.wait_for_frame_and_switch_to_frame(qq_driver, 'tcaptcha_iframe')
    img_element = SeleniumUtils.find_element_by_id(qq_driver, 'slideBg')
    SeleniumUtils.wait_for_element_appeared(qq_driver, img_element)
    big = img_element.get_attribute('src')
    request_download_file_by_url(big, GlobalParam.get_test_image_path() + 'test_qq_mail_big.png')
    img_element = SeleniumUtils.find_element_by_id(qq_driver, 'slideBlock')
    SeleniumUtils.wait_for_element_appeared(qq_driver, img_element)
...
def netcase_163_login(netcase_163_driver, user_name, user_pass):
    ...
    SeleniumUtils.wait_for_frame_and_switch_to_frame(netcase_163_driver, netcase_login_frame)
    SeleniumUtils.wait_for_element_exist(netcase_163_driver, '//input[@name="email"]')
    element = SeleniumUtils.find_element_by_name(netcase_163_driver, 'email')
    element.click()
    element.send_keys(user_name)
    SeleniumUtils.wait_for_element_exist(netcase_163_driver, '//input[@name="password"]')
    element = SeleniumUtils.find_element_by_name(netcase_163_driver, 'password')
    element.click()
    element.send_keys(user_pass)
    element = SeleniumUtils.find_element_by_id(netcase_163_driver, 'dologin')
    element.click()
    # ------------------------security mail captcha not show----------------------
    # SeleniumUtils.wait_for_element_exist(netcase_163_driver,'//div[@class="yidun_panel"]')
    # element = SeleniumUtils.find_element_by_class_name(netcase_163_driver, 'yidun_panel')
    # netcase_163_driver.execute_script("arguments[0].style['display'] = 'block';",element)
    # # element = SeleniumUtils.find_element_by_class_name(netcase_163_driver, 'yidun_bg-img')
    ...
    # time.sleep(4)
    # element = SeleniumUtils.find_element_by_class_name(netcase_163_driver, 'yidun_refresh')
    # element.click()
    #
    # element = SeleniumUtils.find_element_by_class_name(netcase_163_driver, 'yidun_tips__point')
...
def netcase_163_check_mail(netcase_163_driver, mail_sender, mail_title):
    SeleniumUtils.wait_for_element_to_be_clickable(netcase_163_driver, '//div[@id="_mail_component_140_140"]/span[@title="收件箱"]')
    time.sleep(2)
